...
//  console.log(aIndex);
  ...
  var mimetype;
  if(options.html5embed.getCustomMimeService) {
    mimetype = options.html5embed.getCustomMimeService().getMimeType(token.attrs[aIndex][1]);
  }
  if (!mimetype) {
    mimetype = Mimoza.getMimeType(token.attrs[aIndex][1]);
  }
...
module.exports = function html5_embed_plugin(md, options) {
  ...
  if(typeof options.html5embed.getCustomMimeService === "undefined") {
    options.html5embed.getCustomMimeService = options.html5embed.get_custom_mime_service;
  }
